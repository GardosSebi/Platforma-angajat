...
const CATEGORY_QUESTIONS: Partial<Record<SsmTrainingCategoryCode, SsmTrainingTestQuestion[]>> = {
  INTRODUCTORY_GENERAL: [
    {
      id: "SSM-INT-001",
...
export function resolveTrainingTestQuestions(category: SsmTrainingCategoryCode): SsmTrainingTestQuestion[] {
...
export function rebuildPublicQuestions(
  meta: SsmTrainingTestAttemptMeta,
  bank: SsmTrainingTestQuestion[]
): SsmTrainingTestQuestionPublic[] {
  const byId = new Map(bank.map((q) => [q.id, q]));
  return meta.questionIds
    .map((questionId) => {
      const question = byId.get(questionId);
      const permutation = meta.permutations[questionId];
      if (!question || !permutation) return null;
      return {
        id: question.id,
        text: question.text,
        options: permutation.map((index) => question.options[index]!)
      };
    })
    .filter((item): item is SsmTrainingTestQuestionPublic => item != null);
}
