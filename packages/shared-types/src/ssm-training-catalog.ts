...
export interface SsmTrainingCategoryMeta {
  category: SsmTrainingCategory;
  labelRo: string;
  whenPerformed: string;
  legalMinDuration: string;
  defaultLegalHours?: number;
  defaultRecurrenceDays?: number;
  defaultReminderDays: number[];
}

export const SSM_TRAINING_CATEGORY_META: SsmTrainingCategoryMeta[] = [
  {
  ...
  },
  {
    ...
    whenPerformed:
      "Absență >30 zile, accident, echipament nou, modificare proceduri, reluare activitate",
    legalMinDuration: "Minim 8 ore",
    defaultLegalHours: 8,
    defaultReminderDays: [30, 15, 7]
  },
...
export function trainingCategoryLabel(category: SsmTrainingCategory): string {
  return SSM_TRAINING_CATEGORY_META.find((m) => m.category === category)?.labelRo ?? category;
}

export function trainingCategoryMeta(category: SsmTrainingCategory): SsmTrainingCategoryMeta | undefined {
